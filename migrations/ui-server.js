...
function loadCRMMetadata(crm) {
  const metaPath = path.join(__dirname, crm, 'metadata.js');
  ...
  const fieldMapPath = path.join(__dirname, crm, 'fieldMapping.js');
...
function discoverCRMs(domain) {
  return fs.readdirSync(__dirname, { withFileTypes: true })
    .filter(d => d.isDirectory() && fs.existsSync(path.join(__dirname, d.name, 'metadata.js')))
    .map(d => {
  ...
  await new Promise((resolve) => {
    const server = http.createServer(async (req, res) => {
      ...
      if (method === 'GET' && pathname === '/api/crm/connection') {
        ...
        try {
          const AdapterClass = require(`./${CRM}/${crmMeta.adapterModule}`);
      ...
      if (method === 'POST' && pathname === '/api/phases/discover') {
        ...
        setImmediate(async () => {
          try {
            ...
            const discoverPhase = require(`./${CRM}/phases/discover`);
            await discoverPhase({
              domain,
              apiKey,
              adapter,
