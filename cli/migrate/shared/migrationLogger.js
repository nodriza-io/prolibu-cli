...
/**
 * Read the last run timestamp for a specific entity
 * @param {string} domain
 * @param {string} crm
 * @param {string} entity
 * @returns {string|null} ISO timestamp or null
 */
function getLastEntityRun(domain, crm, entity) {
  const log = readLog(domain, crm);
  return log?.entities?.[entity]?.lastRun || null;
}
...
function printSummary(log) {
  ...
  if (entities.length === 0) {
    ...
    for (const [entity, stats] of entities) {
      ...
      const lastRunMsg = stats.lastRun ? ` (last: ${stats.lastRun})` : '';
