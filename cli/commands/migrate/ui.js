...
const fs = require('fs');
const startUIServer = require('../../migrate/ui/ui-server');

/**
 * Auto-discover available CRMs by scanning for folders with a metadata.js file.
 */
function discoverCRMs() {
  const adaptersDir = path.join(__dirname, '../../migrate/adapters');
...
module.exports = async function uiMigration(flags) {
  ...
  const crmLabel = crm ? (() => { try { return require(`../../migrate/adapters/${crm}/metadata`).label; } catch { return crm; } })() : null;
