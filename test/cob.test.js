...
const objectDir = path.join(__dirname, '..', 'accounts', domain, 'objects', 'Cob');
...
async function waitForBackend(maxWaitMs = 60000, intervalMs = 5000) {
  // Initial delay — backend takes ~15s to restart
...
describe('Prolibu CLI - Custom Objects (COB)', () => {
  ...
  describe('Help', () => {
    it('should show help when no command is given', () => {
      const output = execSync('./prolibu cob', { encoding: 'utf8' });
  ...
  describe('List', () => {
    it('should list custom objects from the platform', () => {
      const output = execSync(`./prolibu cob list --domain ${domain}`, { encoding: 'utf8' });
      // May have COBs or not — just should not error
  ...
  describe('Create + Get + Delete lifecycle', () => {
    ...
    it('should create a COB from a JSON file and wait for backend restart', async () => {
      // Ensure backend is ready before attempting create
      await waitForBackend();
      ...
      try {
        const output = execWithRetry(`./prolibu cob create --domain ${domain} -f ${testFile}`);
      ...
      // Wait for backend to restart after COB creation
      await waitForBackend();
    }, 90000);

    it('should get the COB by ID', () => {
      expect(createdId).toBeDefined();
      const output = execSync(`./prolibu cob get --domain ${domain} --id ${createdId}`, { encoding: 'utf8' });
    ...
    it('should delete the COB and wait for backend restart', async () => {
      expect(createdId).toBeDefined();
      // Use yes pipe to auto-confirm deletion
      const output = execSync(`echo "y" | ./prolibu cob delete --domain ${domain} --id ${createdId}`, {
        encoding: 'utf8',
        shell: '/bin/zsh',
      });
      expect(output).toContain('deleted');

      // Wait for backend to restart after COB deletion
      await waitForBackend();
    }, 90000);
  });

  describe('Pull', () => {
    it('should pull custom objects to object/cob/ folder', async () => {
      await waitForBackend();
      const output = execSync(`./prolibu cob pull --domain ${domain}`, { encoding: 'utf8' });
      // May have 0 COBs after the delete, or existing ones
      expect(output).toBeDefined();
    }, 60000);
  });
});
