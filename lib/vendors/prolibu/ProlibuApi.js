...
class ProlibuApi {
  ...
  /**
   * Upsert: Update if exists (by external field), create if not
   ...
   */
  async upsert(entity, externalValue, options = {}, data) {
    if (!data) {
      throw new Error(`Data as fourth argument is required when using upsert`);
    }
