...
class EventManager {
   ...
   * Supports multiple events separated by spaces: 'Event1 Event2 Event3'
   * 
   * @param {string} eventNames - The name(s) of the event(s) to listen for (space-separated for multiple)
  ...
  on(eventNames, handler) {
    // Support multiple events separated by spaces
    const events = eventNames.split(/\s+/).filter(e => e.length > 0);
    
    for (const eventName of events) {
      const modelName = eventName.split('.')[0];
      // check if modelName is different to 'ApiRun'
      if (modelName !== 'ApiRun' && modelName !== 'EndpointRequest' && modelName !== 'ScheduledTask') {
        ...
        if (!lifecycleHooks.includes(modelName)) {
          ...
          continue; // Skip registering this handler, but continue with others
        }
      }
